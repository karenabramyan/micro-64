export default async function sendApplication(user: any, items: any): Promise<void> {
  ...
  const text = `имя заказчика: ${user.login} ${br}email: ${user.email} ${br}телефон заказчика: ${user.phone} ${br}товар: ${items.title} ${br}тип сделки: ${items.type} ${br}стоимость: ${items.price} ${br}тип сделки: ${items.type}`;
